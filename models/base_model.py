...
class Model(ABC):
    ...
    def __init__(self, config: ConfigParser, t_param: ngs.Parameter) -> None:
        ...
        # Now add any new model components.
        # They get added to the mixed finite element space in the order in which they were specified in the config file.
        ...
        self.element = self.config.get_dict(['FINITE ELEMENT SPACE', 'elements'], self.t_param, all_str=True)
        ...
        if len(self.g_D) > 0:
            if len(gfu.components) == 0:  # Single trial functions
                ...
                for component_name in self.g_D.keys():
                    ...
                    gfu.components[i].Set(self.g_D[component_name],
    ...
    def load_mesh_fes(self, mesh=True, fes=True):
        """
    ...
    def construct_and_run_solver(self, a_assembled: ngs.BilinearForm, L_assembled: ngs.LinearForm,
                                 precond: ngs.Preconditioner, gfu: ngs.GridFunction) -> ngs.solvers:
        """
    ...
    def time_derivative_terms(self, gfu_lst: List[ngs.GridFunction], scheme: str)\
            -> Tuple[ngs.CoefficientFunction, ngs.CoefficientFunction]:
        """
        Function to produce the time derivate terms for the linear and bilinear forms.

        Args:
            dt_list: List of timestep sizes ordered from most recent to oldest.
            gfu_lst: List of the solutions of previous time steps ordered from most recent to oldest.
            scheme: The name of the time integration scheme being used

        Returns:
            ~: The terms to add to the linear and bilinear terms, in that order
        """
        U, V = self.get_trial_and_test_functions()

        # List of indicies to ignore for the purpose of calculating time derivative
        ignore_indicies: List[int] = []
            ...
            for val in self.time_derivative_components:
                if not self.time_derivative_components[val]:
                    ...
                    if index is None:
                        ...
                        ignore_indicies.append(cast(int, index))
        ...
        for i in range(len(U)):
            if i not in ignore_indicies:
                # TODO: Figure out a better way to do this so that the string is stored elesewhere. Maybe a name tuple.
                if scheme == 'explicit euler':
                    ...
                    a += (11/6) * U[i] * V[i]
                    L += (3 * gfu_lst[0][i] - 1.5 * gfu_lst[1][i] + 1/3 * gfu_lst[2][i]) * V[i]
                else:
