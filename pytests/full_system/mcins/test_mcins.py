...
@fixture
def purely_convective() -> ConfigParser:
    """
    Fixture to return a ConfigParser loaded with a base config for a transient convection-only problem
    on a corase unit square mesh.
    """
    return ConfigParser('pytests/full_system/mcins/purely_convective/config')
...
class TestDiffusion:
    ...
    def test_implicit_euler_dg(self, capsys: CaptureFixture, diffusion: ConfigParser) -> None:
        ...
        # Run
        expected_errors = [5e-8, 5e-8, 1e-15]
        automated_output_check(capsys, diffusion, expected_errors)
    ...
    def test_adaptive_2_step_cg(self, capsys: CaptureFixture, diffusion: ConfigParser) -> None:
        ...
        # Run
        expected_errors = [8e-8, 8e-8, 3e-16]
        automated_output_check(capsys, diffusion, expected_errors)
    ...
    def test_adaptive_2_step_dg(self, capsys: CaptureFixture, diffusion: ConfigParser) -> None:
        ...
        diffusion['TRANSIENT']['scheme'] = 'adaptive two step'
        # Run
        expected_errors = [5e-8, 5e-8, 3e-14]
        automated_output_check(capsys, diffusion, expected_errors)
...
class TestDiffusionConvection:
    ...
    def test_adaptive_3_step_cg(self, capsys: CaptureFixture, diffusion_convection: ConfigParser) -> None:
        ...
        diffusion_convection['TRANSIENT']['dt_tolerance'] = 'relative -> 0.0 \n absolute -> 1e-6'
        # Run
        expected_errors = [1e-6, 1e-7]
        automated_output_check(capsys, diffusion_convection, expected_errors)
...
class TestFirstOrderRxn:
    ...
    def test_implicit_euler_dg(self, capsys: CaptureFixture, first_order_rxn: ConfigParser) -> None:
        # Change elements
    ...
    def test_crank_nicolson_cg(self, capsys: CaptureFixture, first_order_rxn: ConfigParser) -> None:
        ...
        # Run
        expected_errors = [5.5e-12, 1e-16]
        automated_output_check(capsys, first_order_rxn, expected_errors)

    def test_crank_nicolson_dg(self, capsys: CaptureFixture, first_order_rxn: ConfigParser) -> None:
        # Change elements
        first_order_rxn['FINITE ELEMENT SPACE']['elements'] = 'u -> HDiv\np -> L2\na -> L2\nb -> L2'
        # Change from CG to DG
        first_order_rxn['DG']['DG'] = 'True'
