...
from helpers.io import load_mesh, load_gridfunction_from_file
import numpy as np
...
class DIM:
    ...
    def _load_nonconformal_parameters(self) -> None:
        ...
        self.N_mesh = self.config.get_list(['DIM', 'num_mesh_elements'], int)
        self.scale = self.config.get_list(['DIM', 'mesh_scale'], float)
        self.offset = self.config.get_list(['DIM', 'mesh_offset'], float)

        # Check if phi should be created on a more refined mesh.
        N_tmp = self.config.load_param_simple(['DIM', 'num_phi_mesh_elements'], quiet=True)
        if N_tmp is None:
            # Just use the mesh spacing of the full mesh to create phi.
            self.N = self.N_mesh
        else:
            # Reloading from the config file to take advantage of the type-checking in get_list.
            self.N = self.config.get_list(['DIM', 'num_phi_mesh_elements'], int)
    ...
    def get_DIM_gridfunctions(self, mesh: ngs.Mesh, interp_ord: int):
        ...
        if self.load_method == 'file':
            ...
            self.phi_gfu = load_gridfunction_from_file(phi_filename, fes)

            if self.multiple_bcs:
                # There are BC masks that must be loaded.
                for marker, filename in self.vertices.items():
                    mask_gfu = load_gridfunction_from_file(filename, fes)
            ...
            # Construct the phase field, Grad(phi), and |Grad(phi)| on the refined mesh.
