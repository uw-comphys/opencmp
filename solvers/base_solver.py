"""
Module to for the base solver class
"""
...
class Solver(ABC):
    ...
    def __init__(self, model_class: Type[Model], config: ConfigParser) -> None:
        ...
        if self.transient:
            ...
            if 'adaptive' in self.scheme:
                ...
                dt_tol = self.config.get_list(['TRANSIENT', 'dt_tolerance'], float)
                if len(dt_tol) == 2:
                    # Separate absolute and relative tolerances.
                    self.dt_abs_tol, self.dt_rel_tol = dt_tol
                elif len(dt_tol) == 1:
                    # The absolute and relative tolerance are the same.
                    self.dt_abs_tol = dt_tol[0]
                    self.dt_rel_tol = dt_tol[0]
                else:
                    raise ValueError('Can only specify an absolute and a relative local error tolerance.')
    ...
    def _solve(self) -> None:
        ...
        # Loop until an iteration is accepted, or until the max number of attempes is reached
        while True:
            if self.transient:
    ...
    def reset_model(self) -> None:
        ...
        if self.transient:
            ...
            if self.adaptive:
                self.dt_tol = self.config.get_item(['TRANSIENT', 'dt_tolerance'], float)
                self.dt_range = self.config.get_list(['TRANSIENT', 'dt_range'], float)
                if self.dt_range[0] > self.dt_range[1]:
                    # Make sure dt_range is in the order [min_dt, max_dt]
                    self.dt_range = [self.dt_range[1], self.dt_range[0]]
